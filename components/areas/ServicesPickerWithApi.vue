...
export default {
  ...
  watch: {
    enterData() {
      this.services = this.enterData.services
    } 
  },
  methods: {
     editItem (item) {
      ...
      async deleteItemConfirm () {
        try {
          await this.$axios.delete(`/areas/${this.enterData.id}/${this.editedItem.id}`)
      ...
      async save () {
        if (this.editedIndex > -1) {
          try {
            await this.$axios.put(`/areas/${this.enterData.id}/${this.editedItem.id}`, this.editedItem)
          ...
          try {
            await this.$axios.post(`/areas/${this.enterData.id}/services`, this.editedItem)
